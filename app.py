...
# Convert to string, lowercase, handle variations of 'to' and 'via'
clean_routes = (
    df["Route"].astype(str)
    .str.replace("TO", "to", case=False, regex=True)
    .str.replace(" to ", " to ", regex=False)   # normalize spacing
    .str.replace("via", "-", case=False, regex=True)  # replace 'via' with '-'
)

split_routes = clean_routes.str.split(" to ", n=1, expand=True)

df["Departure"] = split_routes[0].fillna("Unknown").str.strip().str.title()
df["Destination"] = split_routes[1].fillna("Unknown").str.strip().str.title()
...
df["Flight Duration"] = np.where(
    df["Route"].str.contains("London|Paris|Rome", case=False, na=False), "Short-haul",
    np.where(df["Route"].str.contains("New York|Dubai", case=False, na=False), "Long-haul", "Medium-haul")
)
...
departure = st.selectbox("Select Departure:", sorted(df["Departure"].dropna().unique()))
# Destination is filtered automatically based on Departure
available_destinations = sorted(df[df["Departure"] == departure]["Destination"].dropna().unique())
...
filtered_df = filtered_df[
    (filtered_df["Departure"] == departure) &
